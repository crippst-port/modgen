define([], function () {
   ...
   */
  define(['core/modal_events', 'aiplacement_modgen/modal', 'core/str', 'core/templates'], function (ModalEvents, ModgenModal, Str, Templates) {
    ...
    const collectSubmitButtons = form => {
      ...
      submitters.forEach(submitter => {
        ...
        const labelSource = submitter.tagName === 'INPUT' ? submitter.value || submitter.getAttribute('value') || submitter.getAttribute('aria-label') || submitter.name || '' : submitter.textContent;
            ...
            try {
              ...
              for (const pair of formData.entries()) {
                if (pair[1] instanceof File) {
                  ...
                  entries.push({
                    key: pair[0],
                    value: typeof pair[1] === 'string' ? pair[1].slice(0, 200) : String(pair[1])
                  });
                }
              }
    ...
    const setLoadingState = () => {
      ...
      const markup = '' + '<div class="aiplacement-modgen__loading" role="status" aria-live="polite">' + '<span class="spinner-border" aria-hidden="true"></span>' + '<p class="aiplacement-modgen__loading-message"></p>' + '</div>';
